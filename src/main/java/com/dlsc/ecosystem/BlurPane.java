...
public class BlurPane extends StackPane {
	...
	protected void layoutChildren() {
		...
		if (getParent() != null && isVisible()) {
			...
			Rectangle2D toPaint = new Rectangle2D(startPointInScene.getX(),
					startPointInScene.getY(), getLayoutBounds().getWidth(),
					getLayoutBounds().getHeight());
			parameters.setViewport(toPaint);
			WritableImage image = new WritableImage((int) toPaint.getWidth(),
					(int) toPaint.getHeight());
