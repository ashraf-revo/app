...
public class UndecoratorController {
	...
	protected void maximizeOrRestore() {
		...
		if (maximized) {
			...
			savedBounds = new BoundingBox(stage.getX(), stage.getY(),
					stage.getWidth(), stage.getHeight());
	...
	public void saveFullScreenBounds() {
		Stage stage = undecorator.getStage();
		savedFullScreenBounds = new BoundingBox(stage.getX(), stage.getY(),
				stage.getWidth(), stage.getHeight());
	}
	 ...
	 * @param SHADOW
	 */
	public void setStageResizableWith(final Stage stage, final Node node,
			int PADDING, int SHADOW) {
		...
		node.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent mouseEvent) {
				if (undecorator.getStageStyle() != StageStyle.UTILITY
						&& !stage.isFullScreen()
						&& mouseEvent.getClickCount() > 1) {
					if (mouseEvent.getSceneY() - SHADOW_WIDTH < MAXIMIZE_BORDER) {
						undecorator.maximizeProperty().set(
								!undecorator.maximizeProperty().get());
						mouseEvent.consume();
					}
				}
			}
		...
		node.setOnMouseMoved(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent mouseEvent) {
				...
				if (isRightEdge(x, y, boundsInParent)) {
					...
					} else if (y > boundsInParent.getHeight()
		...
		try {
			...
			if (screensForRectangle.size() > 0) {
				...
				if (y < visualBounds.getHeight() - 30
						&& y + SHADOW_WIDTH >= visualBounds.getMinY()) {
					stage.setY(y);
				}
			}
		} catch (Exception e) {
	...
	public boolean isRightEdge(double x, double y, Bounds boundsInParent) {
		if (x < boundsInParent.getWidth()
				&& x > boundsInParent.getWidth() - RESIZE_PADDING
						- SHADOW_WIDTH) {
			return true;
		}
		return false;
	...
	public boolean isBottomEdge(double x, double y, Bounds boundsInParent) {
		if (y < boundsInParent.getHeight()
				&& y > boundsInParent.getHeight() - RESIZE_PADDING
						- SHADOW_WIDTH) {
			return true;
		}
		return false;
